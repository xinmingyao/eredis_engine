...
ERL_NIF_TERM eredis_open(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
 ...
 handle->dbid=0;
  ...
  if (enif_get_string(env, argv[0], name, sizeof(name), ERL_NIF_LATIN1) &&
      enif_get_string(env, argv[1], config, sizeof(config), ERL_NIF_LATIN1)&&enif_get_int(env,argv[2],&is_open)){
    
    if(start_redis(config,name,is_open)>=0){
...
ERL_NIF_TERM eredis_save(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
  ...
  if (enif_get_string(env, argv[1], name, sizeof(name), ERL_NIF_LATIN1)){
    server.rdb_filename=zstrdup(name);
...
ERL_NIF_INIT(eredis_engine, nif_funcs, &on_load, NULL, NULL, NULL);
